...
#include <windows.h>
	...
	#endif
#endif
/* Define MONITORINFOEX ourselves as the WINAPI declarations
* are inconsistant. TCC has the parent MONITORINFO as member mi,
* WINAPI uses a direct include (struct name without a variable name),
* and in C++ they both inherit */
typedef struct tagWSMONITORINFOEXA {
MONITORINFO mi;
CHAR szDevice[CCHDEVICENAME];
} WSMONITORINFOEXA,*LPWSMONITORINFOEXA;

typedef struct tagWSMONITORINFOEXW {
MONITORINFO mi;
WCHAR szDevice[CCHDEVICENAME];
} WSMONITORINFOEXW,*LPWSMONITORINFOEXW;
...
/*=============================================================================
	...
	See "Macworld Mac Programming FAQs" (1996) for how to do this
	pre-Display Manager. Macintosh did not support multiple desktops
	before Display Manager, nor multiple resolutions. It only supported
	a single resolution and 5 depths (1, 2, 4, 8, and 16) spread across
	X number of monitors. Has/SetDepth's 3rd parameter determines if the
	4th parameter is valid, and the 4th parameter sets the monitor
	to grayscale (0) or color (1). The main mointor has a top/left of 0/0
	in it's gdRect. Here's some cleaned up functions mimicing
	the FAQs examples:

	short GetSupportedDepth()
	{
		GDHandle hGD = GetMainDevice();
		if ( !HasDepth(hGD, 2, 0, 0 ) )
			return 1;
		if ( !HasDepth(hGD, 4, 0, 0 ) )
			return 2;
		if ( !HasDepth(hGD, 8, 0, 0 ) )
			return 4;
		if ( !HasDepth(hGD, 16, 0, 0 ) )
			return 8;
		return 16;
	}

	short GetNumGDevices()
	{
		short count = 0;
		GDHandle hDevice;
		for ( hDevice = GetDeviceList(); hDevice != NULL;
	...
	#define klsColorMask 1
	#define kPixelDepthl 128
	#define kPixelDepth2 129
	#define kPixelDepth4 130
	#define kPixelDepth8 131
	#define kPixelDepth16 132

	void GetMonitorMode(GDHandle hGD, short* pixelDepth, Boolean* isColor)
	{
		HLock( (Handle)hGD );
		if ( ((GDPtr)*hGD)->gdFlags & klsColorMask )
			*isColor = TRUE;
		else
			*isColor = FALSE;

		switch ( ((GDPtr)*hGD)->gdMode )
		{
			case kPixelDepthl:
				*pixelDepth = 1;
				break;
	...
	void GetMainGDSize(short* height, short* width)
	{
		GDHandle hGD = GetMainDevice();
		HLock( (Handle)hGD );
		*height = ((GDPtr)*hGD)->gdRect.bottom - ((GDPtr)*hGD)->gdRect.top;
		*width = ((GDPtr)*hGD)->gdRect.right - ((GDPtr)*hGD)->gdRect.left;
		HUnlock( (Handle)hGD );
	}

	if (HasDepth(hGD, 8, 1, 1 ))
		SetDepth(hGD, 8, 1, 1);

	GDHandle GetIntersectingDevices(Rect* testRect)
	{
		long area, maxArea;
		GDHandle hDevice, hMaxintersectingDevice;
		Rect intersection;
		maxArea = 0;
		for (hDevice = GetDeviceList(); hDevice != NULL;
				hDevice = GetNextDevice(hDevice))
		{
			HLock( (Handle)hDevice );
			if (TestDeviceAttribute(hDevice, screenDevice)
					&& TestDeviceAttribute(hDevice, screenActive)
					&& SectRect(testRect,
							&((*hDevice)->gdRect), &intersection))
			{
				area = ((long)(intersection.right - intersection.left))*
					((long)(intersection.bottom - intersection.top));
				if (area > maxArea)
				{
					hMaxlntersectingDevice = hDevice;
					maxArea = area;
				}
			}
			HUnlock( (Handle)hDevice );
		}
		return( hMaxlntersectingDevice );
	}

	void GetWindowDevices( WindowPtr pWin )
	{
		GDHandle hDevice;
		Rect rWindow;
		rWindow = ((*(((WindowPeek)pWin)->strucRgn))->rgnBBox);
		hDevice = GetlntersectingDevices(&rWindow);
	}
...
#endif
#if !defined(GDK_CHECK_VERSION) && defined(GDK_MAJOR_VERSION)
...
#endif
typedef struct WSDISPLAYXDEFDISPLAY
{
	Display* dpy;
	int needsfree;
} WSDISPLAYXDEFDISPLAY;
#if defined(GDK_CHECK_VERSION)
	#if GDK_CHECK_VERSION(3,22,25)
		int WSDisplayGDKIsTypeName(void* instance, const char* string)
		{
	...
	#endif /* GDK 3.22.25 */
#endif
WSDISPLAYHANDLE WSDisplayX11MainDisplay()
{
	...
	return NULL;
		...
		{
		#if GDK_CHECK_VERSION(3,22,25)
			if (WSDisplayGDKIsTypeName(dpy, "GdkX11"))
		#endif
...
void WSDisplayX11DefDisplayCreate(WSDISPLAYXDEFDISPLAY* info)
{
	info->dpy = WSDisplayX11MainDisplay();
	if(!info->dpy)
	{
...
void WSDisplayX11DefDisplayDestroy(WSDISPLAYXDEFDISPLAY* info)
{
	if(info->needsfree)
...
void WSDisplayGetPrimary(WSDISPLAY* pDisplay)
{
	WSDISPLAYXDEFDISPLAY dpy;
	WSDisplayX11DefDisplayCreate(&dpy);
	pDisplay->mHandle = dpy.dpy;
	pDisplay->mX11DisplayNeedsClose = dpy.needsfree;
...
void WSDisplayIterator_Value(const WSDISPLAYITERATOR* pThis,
	WSDISPLAY* pDisplay)
{
	if(pThis->mX11DisplayNeedsClose)
	{ /* "clone" the X display if we opened it */
		WSDISPLAYXDEFDISPLAY dpy;
		WSDisplayX11DefDisplayCreate(&dpy);
		pDisplay->mHandle = dpy.dpy;
		pDisplay->mX11DisplayNeedsClose = dpy.needsfree;
	...
	else
	{
		...
		pDisplay->mX11DisplayNeedsClose = 0;
	}
	pDisplay->mScreenNum =
...
{
	WSDISPLAYXDEFDISPLAY dpy;
	dpy.dpy = pThis->mDisplay;
	dpy.needsfree = pThis->mX11DisplayNeedsClose;
	WSDisplayX11DefDisplayDestroy(&dpy);
...
void WSDisplayBegin(WSDISPLAYITERATOR* pIt)
{
	WSDISPLAYXDEFDISPLAY dpy;
	WSDisplayX11DefDisplayCreate(&dpy);
	pIt->mDisplay = dpy.dpy;
	pIt->mX11DisplayNeedsClose = dpy.needsfree;
...
wsbool WSDisplayGetFromRect(WSRECT* r, WSDISPLAY* pDisplay)
{
	guipos_t x = WSRect_GetLeft(r) + ((WSRect_GetWidth(r)) / 2);
	guipos_t y = WSRect_GetTop(r) + ((WSRect_GetHeight(r)) / 2);
	return WSDisplayGetFromPoint(x, y, pDisplay);
}
wsbool WSDisplayGetFromWindow(void* windowhandle, WSDISPLAY* pDisplay);
...
{
	WSRect_SetTop(r, 0); WSRect_SetLeft(r, 0);
	WSRect_SetRight(r, DisplayWidth(maindisplay, mainscreenidx));
	WSRect_SetBottom(r, DisplayHeight(maindisplay, mainscreenidx));
}
void WSDisplayGetGeometry(WSRECT* r)
{
	WSDISPLAYXDEFDISPLAY dpy;
	WSDisplayX11DefDisplayCreate(&dpy);
	int mainscreenidx = DefaultScreen(dpy.dpy);
	WSDisplayX11GetGeometry_(dpy.dpy, mainscreenidx, r);
	WSDisplayX11DefDisplayDestroy(&dpy);
}
	...
	int screennum, WSRECT* r)
{
	...
	if (prop)
	{
			...
			&& actual_format == 32 && nitems == 4)
		{
			...
			WSRect_SetLeft(r, p[0]);
			WSRect_SetTop(r, p[1]);
			WSRect_SetRight(r, p[2] - WSRect_GetLeft(r));
			WSRect_SetBottom(r, p[3] - WSRect_GetTop(r));
		}
		else
		{
			WSRect_SetLeft(r, 0);
	...
	int screennum, WSRECT* r)
{
	...
	if ( !WSRect_GetRight(r) || !WSRect_GetBottom(r) )
		*r = rgeo; /* WSDisplayX11GetNetWorkArea failed, use full area */
	else
	{ /* Force the client area into the full area */
		if(WSRect_GetRight(r) > WSRect_GetRight(&rgeo))
			WSRect_SetRight(r, WSRect_GetRight(&rgeo));
		if(WSRect_GetLeft(r) < WSRect_GetLeft(&rgeo))
			WSRect_SetLeft(r, WSRect_GetLeft(&rgeo));
		if(WSRect_GetBottom(r) > WSRect_GetBottom(&rgeo))
			WSRect_SetBottom(r, WSRect_GetBottom(&rgeo));
		if(WSRect_GetTop(r) < WSRect_GetTop(&rgeo))
			WSRect_SetTop(r, WSRect_GetTop(&rgeo));
	}
}
void WSDisplayGetClientArea(WSRECT* r)
{
	WSDISPLAYXDEFDISPLAY dpy;
	WSDisplayX11DefDisplayCreate(&dpy);
	WSDisplayX11GetClientArea_(dpy.dpy, DefaultScreen(dpy.dpy), r);
	WSDisplayX11DefDisplayDestroy(&dpy);
}
...
void WSDisplayGetPrimary(WSDISPLAY* pDisplay)
{
	...
	screenResources = XRRGetScreenResources(pDisplay->mHandle, root);
	primaryOutput = XRRGetOutputPrimary(pDisplay->mHandle, root);
	...
	int screennum, WSRECT* r)
{
	...
	WSRect_SetLeft(r, crtcInfo->x);
	WSRect_SetTop(r, crtcInfo->y);
	WSRect_SetRight(r, crtcInfo->x + crtcInfo->width);
	WSRect_SetBottom(r, crtcInfo->y + crtcInfo->height);
